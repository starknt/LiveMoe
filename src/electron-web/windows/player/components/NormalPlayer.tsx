...
const NormalPlayer: FC<PlayerProps> = memo(({ configuration }) => {
  ...
  const renderPlayModeText = useCallback(() => {
    if (configuration.mode === 'random') return '随机播放';

    if (configuration.mode === 'order') return '顺序播放';

    if (configuration.mode === 'single') return '循环播放';

    if (configuration.mode === 'list-loop') return '列表循环';

    return '循环播放';
  }, []);

  const renderPlayModeIcon = useCallback(() => {
    if (configuration.mode === 'random') return <RandomIcon />;

    if (configuration.mode === 'order') return <PlaylistPlayOutlinedIcon />;

    if (configuration.mode === 'list-loop') return <RepeatRoundedIcon />;

    if (configuration.mode === 'single') return <RepeatOneRoundedIcon />;
