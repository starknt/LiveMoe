...
import { DocRes } from 'common/electron-common/database';
  ...
  private handleReady() {
    ...
    this.window.onPlayChange(
      debounce<(e: IWallpaperConfiguration) => void>((configuration) => {
        if (this.cancelPauseToken) this.cancelPauseToken();
    ...
    this.onViewMode(
      debounce(({ x, y }: { x: number; y: number }) => {
        if (win()) {
          if (!this.electronScreen || !this.rtConfiguration.viewMode) return;
